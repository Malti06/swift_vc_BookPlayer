...
class TranscriptFeatureUITests: XCTestCase {
  ...
  func testTranscriptButtonVisibilityDuringAlphaTransitions() throws {
    ...
    // 2. Have its own layer that doesn't inherit alpha
    // 3. Be repositioned outside the artworkControl hierarchy during toggle
    
    // Test the visual state
    ...
    XCTAssertEqual(buttonBackgroundColor, UIColor.systemBlue,
                   "Button background should remain blue, but it's invisible due to parent alpha")
  }
